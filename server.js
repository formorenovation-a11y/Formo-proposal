...
function buildHtml(data, images, proposalId) {
  ...
  // Service-tagged images: category like "service_0", "service_1" etc
  const serviceImgs = idx => images.filter(i=>i.category===`service_${idx}`);
  ...
  function distributeAmounts(cats, total) {
    ...
    const ws = cats.map(c=>wm[c.icon]||0.15);
    const tw = ws.reduce((s,w)=>s+w,0);
    let d = ws.map((w,i)=>({label:cats[i].title,amount:Math.round((w/tw)*total)}));
    const s2 = d.slice(0,-1).reduce((s,x)=>s+x.amount,0);
    d[d.length-1].amount = total-s2;
    return d;
  }
  ...
  // Category label by icon
  const catLabel = icon => ({'countertop':'Surfaces','cabinet':'Cabinetry','paint':'Finishes','trim':'Millwork','plumbing':'Plumbing','flooring':'Flooring','cleanup':'Site Care','demo':'Demo'}[icon]||'Renovation');

  // ── SCOPE CARDS — no photos, just premium text cards ──
  const scopeCards = scopeCategories.map((s,idx) => {
    const sImgs = serviceImgs(idx);
    ...
    <div style="border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02);overflow:hidden">
      ${sImgs.length ? `
      <div style="display:grid;grid-template-columns:repeat(${Math.min(sImgs.length,3)},1fr);gap:2px">
        ${sImgs.slice(0,3).map(img=>`<img src="${img.url}" alt="" style="width:100%;height:200px;object-fit:cover;display:block">`).join('')}
      </div>` : ''}
      <div style="padding:28px 26px">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
          <div style="opacity:0.7;flex-shrink:0">${icon}</div>
          <div>
            <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A96E;margin-bottom:4px">${catLabel(s.icon)}</div>
  ...
  // ── COMPARATIVA: render images = "After", ba images = "Today" ──
  // Build pairs: first ba image + first render, second ba + second render, etc.
  const comparePairs = [];
  const maxPairs = Math.max(baImgs.length, renderImgs.length);
  for (let i=0; i<maxPairs; i++) {
    comparePairs.push({ before: baImgs[i]||null, after: renderImgs[i]||null });
  }

  const compareSection = comparePairs.length ? `
<section style="padding:80px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <div style="font-size:14px;color:#b8b0a2;margin-bottom:44px;max-width:560px;line-height:1.7">A side-by-side look at your space as it stands today, and the elevated result we will deliver.</div>
    ${comparePairs.map((p,i)=>`
    <div style="margin-bottom:4px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:3px">
        ${p.before ? `
        <div style="position:relative">
          <img src="${p.before.url}" alt="Today" style="width:100%;display:block;max-height:520px;object-fit:contain;background:#111">
          <div style="position:absolute;top:16px;left:16px;background:rgba(25,24,22,0.85);padding:7px 16px;font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#b8b0a2;border:1px solid rgba(255,255,255,0.12)">Today</div>
        </div>` : `<div style="background:#111;min-height:300px;display:flex;align-items:center;justify-content:center"><span style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#3a3830">No photo</span></div>`}
        ${p.after ? `
        <div style="position:relative">
          <img src="${p.after.url}" alt="After Renovation" style="width:100%;display:block;max-height:520px;object-fit:contain;background:#111">
          <div style="position:absolute;top:16px;left:16px;background:rgba(201,169,110,0.15);padding:7px 16px;font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A96E;border:1px solid rgba(201,169,110,0.4)">After Renovation</div>
        </div>` : `<div style="background:#111;min-height:300px;display:flex;align-items:center;justify-content:center;border:1px dashed rgba(201,169,110,0.2)"><span style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#C9A96E;opacity:0.5">Coming Soon</span></div>`}
      </div>
    </div>`).join('')}
  </div>
</section>
...
@media(max-width:768px){
  ...
  .ml-grid{grid-template-columns:1fr 1fr!important}
...
<section style="padding:80px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <div style="font-size:14px;color:#b8b0a2;margin-bottom:40px;max-width:560px;line-height:1.7">A transparent breakdown by service — priced by what we deliver, not by line-item costs.</div>
...
<section style="padding:80px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <div class="ml-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:3px">
      <div style="padding:32px 24px;border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02)">
        <div style="margin-bottom:16px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="34" height="34"><rect x="4" y="8" width="30" height="22" rx="1.5" stroke="#C9A96E" stroke-width="1.5"/><path d="M4 14h30M11 8v6M27 8v6" stroke="#C9A96E" stroke-width="1.4" stroke-linecap="round"/><circle cx="19" cy="22" r="2.5" stroke="#C9A96E" stroke-width="1.3"/></svg></div>
        <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A96E;margin-bottom:8px">Materials</div>
        <div style="font-family:'Cormorant Garamond',serif;font-size:19px;font-weight:600;color:#F8F4EF;margin-bottom:10px">Premium Materials</div>
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">All paints, primers, adhesives, hardware, and finishing supplies sourced at professional grade — included in your investment.</div>
      </div>
      <div style="padding:32px 24px;border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02)">
        <div style="margin-bottom:16px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="34" height="34"><rect x="3" y="14" width="26" height="16" rx="1.5" stroke="#C9A96E" stroke-width="1.5"/><path d="M29 20h4l2 4v4h-6" stroke="#C9A96E" stroke-width="1.4" stroke-linejoin="round"/><circle cx="10" cy="32" r="2.5" stroke="#C9A96E" stroke-width="1.3"/><circle cx="24" cy="32" r="2.5" stroke="#C9A96E" stroke-width="1.3"/><path d="M8 14V9a3 3 0 013-3h6a3 3 0 013 3v5" stroke="#C9A96E" stroke-width="1.4"/></svg></div>
        <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A96E;margin-bottom:8px">Logistics</div>
        <div style="font-family:'Cormorant Garamond',serif;font-size:19px;font-weight:600;color:#F8F4EF;margin-bottom:10px">Delivery &amp; Transport</div>
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">All material delivery, tool transport, and crew logistics fully coordinated. You don't lift a finger.</div>
      </div>
      <div style="padding:32px 24px;border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02)">
        <div style="margin-bottom:16px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="34" height="34"><path d="M10 28l14-14M10 26L8 28l2-2z" stroke="#C9A96E" stroke-width="1.5" stroke-linecap="round"/><path d="M20 8l10 10-4 2-8-8 2-4z" stroke="#C9A96E" stroke-width="1.4" stroke-linejoin="round"/></svg></div>
        <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A96E;margin-bottom:8px">Labor</div>
        <div style="font-family:'Cormorant Garamond',serif;font-size:19px;font-weight:600;color:#F8F4EF;margin-bottom:10px">Skilled Labor</div>
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">Experienced crew handles every trade involved — fully supervised and held to the highest standard of finish quality.</div>
      </div>
      <div style="padding:32px 24px;border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02)">
        <div style="margin-bottom:16px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="34" height="34"><path d="M12 16l-4 14h22l-4-14" stroke="#C9A96E" stroke-width="1.5" stroke-linejoin="round"/><path d="M19 7v9M11 11l8-4 8 4" stroke="#C9A96E" stroke-width="1.4" stroke-linecap="round"/></svg></div>
        <div style="font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#C9A96E;margin-bottom:8px">Site Care</div>
        <div style="font-family:'Cormorant Garamond',serif;font-size:19px;font-weight:600;color:#F8F4EF;margin-bottom:10px">Protection &amp; Cleanup</div>
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">Surfaces protected throughout, debris removed daily, and a full post-project cleanup before we hand your space back.</div>
      </div>
    </div>
  </div>
</section>
  ...
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <div class="why-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px">
      <div style="padding:36px 28px;border:1px solid rgba(201,169,110,0.25);background:rgba(255,255,255,0.02);text-align:center">
        ...
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">Fully licensed and insured on every project — complete peace of mind throughout your renovation.</div>
      </div>
      <div style="padding:36px 28px;border:1px solid rgba(201,169,110,0.25);background:rgba(255,255,255,0.02);text-align:center">
        ...
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">Clear milestones, daily communication, and a genuine commitment to your timeline.</div>
      </div>
      <div style="padding:36px 28px;border:1px solid rgba(201,169,110,0.25);background:rgba(255,255,255,0.02);text-align:center">
        ...
        <div style="font-size:13px;color:#b8b0a2;line-height:1.7">Master-level execution with meticulous attention to every detail. We don't settle — we deliver exceptional.</div>
      </div>
    </div>
  </div>
</section>
