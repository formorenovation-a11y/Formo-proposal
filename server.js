...
function buildHtml(data, images, proposalId) {
  ...
  const scopeCards = scopeCategories.map((s,idx)=>{
    ...
    <div style="border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02);overflow:hidden">
      ${imgs.length?`<div style="display:grid;grid-template-columns:repeat(${Math.min(imgs.length,3)},1fr);gap:2px">${imgs.map(i=>`<img src="${i.url}" alt="" style="width:100%;display:block;max-height:480px;object-fit:contain;background:#111">`).join('')}</div>`:''}
