...
function buildHtml(data, images, proposalId) {
  ...
  // Distribute grandTotal across scope categories proportionally
  // Use line items as weight basis, then relabel by service name
  const total = Number(grandTotal) || 0;
  const scopeCount = scopeCategories.length;
  
  // Smart distribution: assign amounts based on service type
  function distributeAmounts(categories, total) {
    if (!categories.length) return [];
    // Rough weights by typical renovation cost structure
    const weightMap = {
      'countertop': 0.30, 'cabinet': 0.28, 'paint': 0.18,
      'trim': 0.10, 'plumbing': 0.06, 'cleanup': 0.04,
      'flooring': 0.25, 'demo': 0.08, 'general': 0.15
    };
    const weights = categories.map(c => weightMap[c.icon] || 0.15);
    const totalWeight = weights.reduce((s,w)=>s+w, 0);
    let distributed = weights.map((w,i) => ({
      label: categories[i].title,
      amount: Math.round((w/totalWeight) * total)
    }));
    // Adjust last item so sum equals exact total
    const sum = distributed.slice(0,-1).reduce((s,d)=>s+d.amount,0);
    distributed[distributed.length-1].amount = total - sum;
    return distributed;
  }

  // Use provided lineItems if they look like service categories (not labor/materials breakdown)
  // Otherwise distribute by scope
  const isServiceBased = lineItems.length > 0 && !lineItems.some(l =>
    /^labor|^material|^suppli|^subcon|^debris|^dump/i.test(l.label)
  );
  const displayItems = isServiceBased
    ? lineItems
    : distributeAmounts(scopeCategories, total);
  ...
  // 2-per-row renders, larger height
    ...
    <div style="padding:80px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
      <div style="max-width:1100px;margin:0 auto;padding:0 6%">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          ${renderImgs.map(i=>`<img src="${i.url}" style="width:100%;height:420px;object-fit:cover;border:1px solid rgba(201,169,110,0.2);display:block" loading="lazy">`).join('')}
        </div>
      </div>
    </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          ${baImgs.map((img,i)=>`
            <div style="position:relative">
              <img src="${img.url}" style="width:100%;height:400px;object-fit:cover;border:1px solid rgba(201,169,110,0.2);display:block">
...
@media(max-width:768px){
  ...
  .inv-row-pad{padding:14px 20px!important}
  .inv-total-pad{padding:20px!important}
}
</style>
</head>
<body>
...
<section style="position:relative;min-height:100vh;display:flex;align-items:center;justify-content:center;text-align:center;overflow:hidden">
  ...
  <div style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(25,24,22,0.45) 0%,rgba(25,24,22,0.28) 35%,rgba(25,24,22,0.65) 100%)"></div>
  <div style="position:relative;z-index:1;padding:80px 6%;max-width:900px;width:100%">
    <div style="font-size:11px;letter-spacing:5px;text-transform:uppercase;color:#C9A96E;opacity:0.9;margin-bottom:36px">Formo Renovation</div>
    <div style="font-size:11px;letter-spacing:3px;text-transform:uppercase;color:rgba(248,244,239,0.55);margin-bottom:18px">Prepared For</div>
    <h1 class="hero-name" style="font-family:'Cormorant Garamond',serif;font-size:clamp(50px,8vw,90px);font-weight:300;letter-spacing:2px;line-height:1;color:#F8F4EF;margin-bottom:22px;text-shadow:0 2px 20px rgba(0,0,0,0.4)">${esc(clientName)}</h1>
    <div style="width:48px;height:1px;background:#C9A96E;margin:0 auto 22px;opacity:0.6"></div>
    <div style="font-family:'Cormorant Garamond',serif;font-size:clamp(18px,2.5vw,24px);font-style:italic;color:#C9A96E;margin-bottom:14px">${esc(projectTitle)}</div>
    <div style="font-size:11px;letter-spacing:3px;text-transform:uppercase;color:rgba(248,244,239,0.5);margin-bottom:28px">${esc(projectAddress)}</div>
    <div style="font-family:'Cormorant Garamond',serif;font-size:14px;font-style:italic;color:rgba(248,244,239,0.4)">"Built on quality, driven by integrity"</div>
  </div>
  <div style="position:absolute;bottom:32px;left:50%;transform:translateX(-50%);color:#C9A96E;font-size:20px;opacity:0.5">↓</div>
</section>
...
<section style="padding:80px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <div style="max-width:660px;border:1px solid rgba(201,169,110,0.28);background:rgba(255,255,255,0.02)">
      ...
      ${invRows}
      <div class="inv-total-pad" style="padding:28px 36px;display:flex;justify-content:space-between;align-items:center;border-top:1px solid rgba(201,169,110,0.2)">
  ...
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <div class="why-grid" style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px">
      <div style="padding:34px 26px;border:1px solid rgba(201,169,110,0.28);background:rgba(255,255,255,0.02);text-align:center">
        <div style="margin:0 auto 16px;width:38px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="38" height="38"><circle cx="19" cy="19" r="16" stroke="#C9A96E" stroke-width="1.5"/><path d="M12 19l5 5 9-9" stroke="#C9A96E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></div>
      ...
      <div style="padding:34px 26px;border:1px solid rgba(201,169,110,0.28);background:rgba(255,255,255,0.02);text-align:center">
        <div style="margin:0 auto 16px;width:38px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="38" height="38"><circle cx="19" cy="19" r="16" stroke="#C9A96E" stroke-width="1.5"/><path d="M19 10v9l6 3" stroke="#C9A96E" stroke-width="1.5" stroke-linecap="round"/></svg></div>
      ...
      <div style="padding:34px 26px;border:1px solid rgba(201,169,110,0.28);background:rgba(255,255,255,0.02);text-align:center">
        <div style="margin:0 auto 16px;width:38px;opacity:0.75"><svg viewBox="0 0 38 38" fill="none" width="38" height="38"><circle cx="19" cy="19" r="16" stroke="#C9A96E" stroke-width="1.5"/><path d="M19 13l2.5 5h5.5l-4.5 3.5 1.5 5.5L19 24l-4.5 3 1.5-5.5L11.5 18h5.5z" stroke="#C9A96E" stroke-width="1.4" stroke-linejoin="round"/></svg></div>
...
<section style="padding:100px 0;text-align:center">
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    <h2 style="font-family:'Cormorant Garamond',serif;font-size:clamp(28px,4vw,52px);font-weight:300;color:#F8F4EF;margin-bottom:12px">Ready to get started?</h2>
    ...
    <div class="cta-box" style="display:inline-block;text-align:left;border:1px solid rgba(201,169,110,0.28);padding:36px 52px;background:rgba(255,255,255,0.02)">
      <div style="font-family:'Cormorant Garamond',serif;font-size:28px;font-weight:600;color:#F8F4EF;margin-bottom:18px">Dan Lares</div>
