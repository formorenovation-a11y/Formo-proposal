...
app.delete('/api/proposals/:id', async (req,res) => {
  ...
  try {
    // Remove from RAM
    delete RAM.tracking[id];
    delete RAM.proposals[id];
    // Remove from Firestore
    await db.collection('proposals').doc(id).delete();
    // Remove HTML from Storage
    try { await bucket.file(`proposals/${id}.html`).delete(); } catch(e) {}
...
function buildHtml(data, images, proposalId) {
  ...
  const scopeCards = scopeCategories.map((s,idx)=>{
    ...
    <div style="border:1px solid rgba(201,169,110,0.22);background:rgba(255,255,255,0.02);overflow:hidden">
      ${imgs.length?`<div style="display:grid;grid-template-columns:repeat(${Math.min(imgs.length,3)},1fr);gap:2px">${imgs.map(i=>`<img src="${i.url}" alt="" style="width:100%;aspect-ratio:4/3;object-fit:cover;display:block">`).join('')}</div>`:''}
...
<section style="padding:80px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
  <div style="max-width:1100px;margin:0 auto;padding:0 6%">
    ...
    ${Array.from({length:maxPairs},(_,i)=>{
      ...
      return `<div class="compare-row" style="display:grid;grid-template-columns:1fr 1fr;gap:3px;margin-bottom:3px">
        ${before
...
<div style="position:fixed;bottom:24px;right:24px;z-index:999">
  <button onclick="downloadProposal()" style="background:#C9A96E;color:#191816;border:none;font-family:'Jost',sans-serif;font-size:11px;font-weight:600;letter-spacing:2px;text-transform:uppercase;padding:12px 20px;cursor:pointer;box-shadow:0 4px 20px rgba(0,0,0,0.4);display:flex;align-items:center;gap:8px">
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1v8M4 6l3 3 3-3" stroke="#191816" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/><path d="M1 10v1a2 2 0 002 2h8a2 2 0 002-2v-1" stroke="#191816" stroke-width="1.4" stroke-linecap="round"/></svg>
    Download Proposal
  </button>
</div>
<script>
function downloadProposal() {
  ...
  a.download = '${esc(clientName).replace(/[^a-zA-Z0-9]/g,'-')}-Formo-Proposal.html';
  a.click();
}
</script>`;
...
@media(max-width:768px){
  ...
  .ml{grid-template-columns:1fr 1fr!important}
  .ia{font-size:42px!important}
  .hn{font-size:46px!important}
  .cb{padding:28px 24px!important}
  .compare-row{grid-template-columns:1fr!important}
}
@media(max-width:480px){
  .ml{grid-template-columns:1fr!important}
  .compare-row{grid-template-columns:1fr!important}
}
</style>
</head>
